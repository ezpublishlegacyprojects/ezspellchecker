...
//---- classes -----------------------------------------------------------------
  ...
  */
  class cpaint {
    ...
    */
    function __construct( $pspell_link ) {
    ...
    */
    function start($input_encoding = 'UTF-8') {
      ...
      if (!isset($_REQUEST['api_query'])
        ...
        $system_arguments[] = $this;
        $system_arguments[] = $this->pspell_link;
        $arguments = cpaint_transformer::decode_array($arguments, $this->basenode->get_encoding());
        $arguments = array_merge( $system_arguments, $arguments );
        ...
        if (is_array($this->api_functions[$user_function])
          ...
          $this->basenode->set_data('[CPAINT] ' . $user_function . 'A function name was passed that is not allowed to execute on this server.');
        }
      } // end: if
    }
